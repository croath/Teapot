//! Antigravity local OAuth redirect listener (`/oauth-callback`).
...
/// Authorization code + state from Google OAuth redirect.
#[derive(Debug, Clone)]
...
impl AntigravityProvider {
  /// Listen on `127.0.0.1:port` for Antigravity redirect to the given path.
  pub(super) async fn wait_for_callback(
      ...
      get(move |Query(q): Query<CallbackQuery>| {
        ...
        async move {
          ...
          let html = match &result {
            ...
            <h1>Antigravity authentication failed</h1><p>{e}</p></body></html>"#
            ),
          };
    ...
    let listener = tokio::net::TcpListener::bind(addr)
      .await
      .map_err(|e| AppError::Internal(format!("antigravity: bind OAuth callback {addr}: {e}")))?;
    let bound = listener
      .local_addr()
      .map_err(|e| AppError::Internal(format!("local_addr: {e}")))?;
    info!(%bound, path = %path, "antigravity OAuth callback listening");
    ...
    match outcome {
      ...
      Ok(Err(_)) => Err(AppError::Internal(
        "antigravity: OAuth callback channel closed unexpectedly".into(),
      )),
      Err(_) => Err(AppError::Unauthorized(
        "antigravity: timed out waiting for OAuth callback".into(),
      )),
    }
  }
}
...
<html><head><meta charset="utf-8"><title>Teapot — Antigravity</title></head>
